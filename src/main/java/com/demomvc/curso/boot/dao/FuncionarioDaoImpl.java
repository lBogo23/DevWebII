...
public class FuncionarioDaoImpl extends AbstractDao<Funcionario, Long> implements FuncionarioDao{
		...
		public List<Funcionario> findByCargoId(Long id) {
			...
			return createQuery("select f from Funcionario f where f.cargo.id = ?1", id); // ?1 ̩ substituido pelo par̢metro id
		}
		...
		public List<Funcionario> findByDataEntradaDataSaida(LocalDate entrada, LocalDate saida) {
			String jpql = new StringBuilder("select f from Funcionario f ")
					...
					.append("order by f.dataEntrada asc") // ?1 par̢metro dataEntrada ?2 par̢metro dataSaida
					.toString();
